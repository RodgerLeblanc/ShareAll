...
bool FileSaver::save(QString text) {
    ...
    QFile thisFile("/accounts/1000/shared/books/" + QDateTime::currentDateTime().toString("yy-MM-dd-hh:mm:ss") + ".txt");
...
bool FileSaver::save(QString text, QString fileName) {
    ...
    QFile thisFile("/accounts/1000/shared/books/" + fileName + ".doc");
    qDebug() << thisFile.fileName();
    if (thisFile.open(QIODevice::ReadWrite))
    {
        // Write to the file
